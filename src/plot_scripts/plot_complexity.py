...
def plot_complexity(matrix, title, save_path):
    ...
    x_labels_a = [r"$P_{\theta}(s_{t}|s_{t-1}^{neg}, u_{t-1}^{tumble})$",
                  r"$P_{\theta}(s_{t}|s_{t-1}^{pos}, u_{t-1}^{tumble})$"]

    x_labels_b = [r"$P_{\theta}(s_{t}|s_{t-1}^{neg}, u_{t-1}^{run})$",
...
if __name__ == "__main__":
    ...
    plot_complexity(reversed_complexity, "Change in distributions (reversed prior)", REVERSED_COMPLEXITY)
