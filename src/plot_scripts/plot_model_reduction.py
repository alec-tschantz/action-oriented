...
def create_heatmap(matrix, title, save_path, color_bar=False):
    ...
    g1 = sns.heatmap(matrix[0, :, :] * 100, cmap="OrRd", ax=ax1, vmin=0.0, vmax=100.0, linewidth=2.5, annot=True,
                     xticklabels=x_labels, yticklabels=y_labels, cbar=color_bar)
    g2 = sns.heatmap(matrix[1, :, :] * 100, cmap="OrRd", ax=ax2, vmin=0.0, vmax=100.0, linewidth=2.5, annot=True,
                     xticklabels=x_labels, yticklabels=y_labels, cbar=color_bar)
...
if __name__ == "__main__":
    ...
    ax.bar(range(N_AGENTS), [full_total, inst_total, epis_total, rand_total], color=colors, edgecolor='grey',
           align='center')
    plt.xticks(x_ticks)
